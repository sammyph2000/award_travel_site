...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function plotHotelLocations(hotelData) {
        var infoWindow = new google.maps.InfoWindow(); // Single InfoWindow instance reused for efficiency
    
        // Close the infoWindow when the map is clicked (not on a marker)
        ...
        hotelData.forEach(function(hotel) {
            ...
            // Define marker color based on parent_co
            var markerColors = {
                marriott: "orange",
                hyatt: "royalblue",
                ihg: "grey",
            ...
            var markerColor = markerColors[hotel.parent_co] || "red"; // Default to red if no match
            ...
            var parentCoTitleCase = hotel.parent_co.charAt(0).toUpperCase() + hotel.parent_co.slice(1); // Title case conversion
            var cashPerNight = hotel.prices.cash && hotel.prices.cash.cash_per_night != null ? '$' + hotel.prices.cash.cash_per_night.toFixed(2) : 'Unknown';
            var pointsPerNight = hotel.prices.points && hotel.prices.points.avg_cost_per_night != null ? hotel.prices.points.avg_cost_per_night.toLocaleString() + ' points' : 'Unknown';
            var imageUrl = hotel.image_urls && hotel.image_urls.length > 0 ? hotel.image_urls[0] : 'path/to/default/image.jpg';
            ...
            var contentString = '<div style="border: 3px solid #3bdb6b; border-radius: 20px; overflow: hidden; display: flex; cursor: pointer; box-shadow: 0px 2px 4px rgba(0,0,0,0.3);">' +
                                '<div style="flex: 1; min-width: 100px;"><img src="' + imageUrl + '" style="width: 100%; height: auto;"></div>' +
                                '<div style="flex: 2; padding: 10px; display: flex; flex-direction: column; justify-content: space-between;">' +
                                '<div>' +
                                '<div style="font-weight: bold; font-size: 16px;">' + hotel.hotel_name + '</div>' +
                                '<div>' + parentCoTitleCase + '</div>' +
                                '<div>Cash per night: ' + cashPerNight + '</div>' +
                                '<div>Points per night: ' + pointsPerNight + '</div>' +
                                '</div>' +
                                `<a href="${bookNowUrl}" target="_blank" style="background-color: #3bdb6b; color: black; text-align: center; padding: 10px; margin-top: 10px; text-decoration: none; font-weight: bold;">Book Now</a>` +
                                '</div>' +
                                '</div>';

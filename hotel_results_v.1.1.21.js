...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function centerMapOnDestination(destination) {
        if (!mapInitialized) {
            ...
            setTimeout(() => centerMapOnDestination(destination), 500);
            return;
        }
    ...
    function plotHotelLocations(hotelData) {
        ...
        hotelData.forEach(function(hotel) {
            // Ensure coordinates exist and are not an empty string before proceeding
            if (!hotel.coordinates || typeof hotel.coordinates !== 'string' || hotel.coordinates.trim() === '') {
                console.error('Hotel coordinates missing or invalid for hotel:', hotel.hotel_name);
            ...
            var contentString = getContentString(hotel);
    ...
    function getMarkerColor(parent_co) {
        ...
        return markerColors[parent_co] || "red";
    }

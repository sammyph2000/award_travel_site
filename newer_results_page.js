...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyBrandSpecialAndPointFilters(hotel, brandFilters, specialFilters, pointFilters) {
        ...
        // If no filters are checked, return true to include all hotels
    ...
    function applySpecialFilters(hotel, specialFilters) {
        const transferPartners = {
            ...
            "chase-ur": ["ihg", "marriott", "hyatt"],
            "cap-one": ["wyndham", "accor"],
            "citi-typ": ["accor", "choice", "wyndham"],
            "bilt": ["marriott", "ihg", "hyatt"]
        };

        return Object.entries(specialFilters).some(([filterKey, isChecked]) => {
            if (isChecked) {
                return transferPartners[filterKey] && transferPartners[filterKey].includes(hotel.parent_co);
            }
            return false;
        });
    }
